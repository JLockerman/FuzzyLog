...
pub mod test {
    ...
    fn multi_server_threaded_multiput() {
        ...
        let mut upcalls: HashMap<_, Box<for<'r> Fn(&'r _) -> bool>> = HashMap::new();
        let mut upcalls2: HashMap<_, Box<for<'r> Fn(&'r _) -> bool>> = HashMap::new();
        let populate_upcalls = |up: &mut HashMap<_, Box<for<'r> Fn(&'r _) -> bool>>| {
            let map = Rc::new(RefCell::new(HashMap::new()));
            let re1 = map.clone();
            let re2 = map.clone();
            up.insert(17.into(), Box::new(move |&(k, v)| {
                trace!("MapEntry({:?}, {:?})", k, v);
                re1.borrow_mut().insert(k, v);
                true
            }));
            up.insert(18.into(), Box::new(move |&(k, v)| {
                trace!("MapEntry({:?}, {:?})", k, v);
