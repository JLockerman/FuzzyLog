...
type L6Edge<V> = Option<Unique<[L7Edge<V>; ARRAY_SIZE]>>;
type L7Edge<V> = Option<Unique<[ValEdge<V>; ARRAY_SIZE]>>;
type ValEdge<V> = Option<Unique<V>>;

const ARRAY_SIZE: usize = 256;
const MASK: u64 = ARRAY_SIZE as u64 - 1;
const SHIFT_LEN: u8 = 8;
// 1...7
...
impl<V> Trie<V> {
    ...
    pub fn insert(&mut self, k: u64, v: V) -> Option<V> {
        // 70
        unsafe {
            let root_index = ((k >> (SHIFT_LEN *7)) & MASK) as usize;
            ...
            let l7 = insert!(l6, k, 6);
            let val_ptr = insert!(l7, k, 7);
            if (*val_ptr).is_none() {
                let ptr = Box::new(v);
    ...
    pub fn entry(&mut self, k: u64) -> Entry<V> {
        unsafe {
            ...
            let l7 = entry!(l6, k, 6, L6);
            let val_ptr = entry!(l7, k, 7, L7);
            match *val_ptr {
...
impl<'a, V: 'a> Entry<'a, V> {
    pub fn insert_with<F: FnOnce() -> V>(self, v: F) -> &'a mut V {
        use self::Entry::*;
        match self {
            Occupied(e) => e.insert_with(v),
            Vacant(e) => e.insert_with(v),
        }
    }
...
macro_rules! fill_entry {
    ...
    ($array:ident, $k:expr, $v:expr, 6) => {
        {
            let l: &mut L7Edge<_> = fill_entry!($array, $k, 6, body);
            fill_entry!(l, $k, $v, 7)
        }
    };
    ($array:ident, $k:expr, $v:expr, 7) => {
        {
            let slot: &mut ValEdge<_> = fill_entry!($array, $k, 7, body);
            //TODO
...
#[test]
pub fn size() {
    assert_eq!(mem::size_of::<[Trie<u8>; ARRAY_SIZE]>(), 2048);
}
