...
use super::{Entry, MutEntry, EntryContents, EntryContentsMut};
...
impl Buffer {
    ...
    pub fn to_sentinel(&mut self) -> bool {
        ...
        super::slice_to_sentinel(&mut self.inner[..])
        //self.inner[0] = unsafe { ::std::mem::transmute(EntryKind::Sentinel) }
    }

    pub fn from_sentinel(&mut self, was_multi: bool) {
        if was_multi {
            super::slice_to_multi(&mut self.inner[..])
        }
    }

    pub fn skeens1_rep_from_sentinel(&mut self, was_multi: bool) {
        if was_multi {
            super::slice_to_skeens1_multirep(&mut self.inner[..])
        } else {
            super::slice_to_skeens1_sentirep(&mut self.inner[..])
        }
    }
